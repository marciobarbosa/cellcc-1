...
use AFS::CellCC::DB qw(db_rw find_update_jobs update_job job_error describe_jobs update_jobs);
...
process_dumps($$$@) {
    ...
    for my $volname (sort keys %vol_table) {
        for my $lastupdate (sort keys %{$vol_table{$volname}}) {
            ...
            my @ids = get_jobs_info("jobid", @jbs);
            my $pid = $pm->start();
            if ($pid) {
                # In parent
            ...
            eval {
                eval {
                    _do_dump($server, $start_state, @jbs);
                };
                if ($@) {
                    my $error = $@;
